class Bolton::SubfamilyCatalog < Bolton::Catalog
  ...
  def parse_extinct_genera_incertae_sedis_in_subfamily_list subfamily
    return unless @type == :extinct_genera_incertae_sedis_in_subfamily_list
    @parse_result[:genera].each do |genus|
      Genus.create! :name => genus, :subfamily => subfamily, :fossil => true, :incertae_sedis_in => 'subfamily', :status => 'valid'
    end
  end
