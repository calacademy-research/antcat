...
if Rails.env.development?
  namespace :db do
    desc "Download and import latest db dump from EngineYard (shortcut: `rake db:il`)"
    task :import_latest do
      sh "RAILS_ENV=development ./script/db_dump/import_latest"
    end
    task il: :import_latest # Shortcut.

    desc "Like `rake db:import_latest`, but without PaperTrail versions (shortcut: `rake db:ilq`)"
