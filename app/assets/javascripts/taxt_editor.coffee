  ...
  $(HISTORY_ITEM_SAVE_BUTTONS).click (event) ->
    ...
    data =
      history_item:
        ...
        object_protonym_id: taxtEditor.find('select[name=object_protonym_id]').val()
        object_taxon_id: taxtEditor.find('select[name=object_taxon_id]').val()
        reference_id: taxtEditor.find('select[name=reference_id]').val()
