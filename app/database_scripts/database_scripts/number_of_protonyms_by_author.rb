...
module DatabaseScripts
  class NumberOfProtonymsByAuthor < DatabaseScript
    ...
    #   ... FROM `authors` LEFT JOIN `author_names` names ON ...
    #   LEFT JOIN `names` `names_protonyms` ON ... WHERE `names`.`type` IN ...
    #
    #   author_join.where(names_protonyms: { type: 'X' })
    #   ... FROM `authors` LEFT JOIN `author_names` ON ...
    #   LEFT JOIN `names` ON ... WHERE `names_protonyms`.`type` = 'X'
    # ```
    def results
