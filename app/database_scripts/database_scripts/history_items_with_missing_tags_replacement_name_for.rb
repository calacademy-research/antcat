...
module DatabaseScripts
  class HistoryItemsWithMissingTagsReplacementNameFor < DatabaseScript
    LIMIT = 250

    def results
      TaxonHistoryItem.where('taxt REGEXP ?', "^Replacement name for #{Taxt::MISSING_TAG_START}2").limit(LIMIT)
    end
...
title: History items with <code>missing</code> tags (replacement name for)
