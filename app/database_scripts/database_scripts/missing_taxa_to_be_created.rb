  ...
  class MissingTaxaToBeCreated < DatabaseScript
    ...
    private
      ...
      def all_hardcoded_taxts
        HistoryItem.taxts_only.where('taxt LIKE ?', "%{#{Taxt::MISSING_TAG}%").pluck(:taxt).join
      end

      def new_taxon_link quick_adder
        label = "Add #{quick_adder.taxon_class.name}"
        link_to label, new_taxon_path(quick_adder.taxon_form_params), class: "btn-default btn-tiny"
      end
  end
end
