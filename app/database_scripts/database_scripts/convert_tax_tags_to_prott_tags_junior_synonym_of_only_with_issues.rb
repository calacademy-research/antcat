...
module DatabaseScripts
  class ConvertTaxTagsToProttTagsJuniorSynonymOfOnlyWithIssues < DatabaseScript
    def statistics
    end

    def results
      HistoryItem.where('taxt REGEXP ?', "^Junior synonym of {tax [0-9]+}: {ref [0-9]+}: [0-9]+\.?$")
    end
...
title: Convert <code>tax</code> tags to <code>prott</code> tags (junior synonym of, only with issues)
...
related_scripts:
  - ConvertTaxTagsToProttTagsJuniorSynonymOfOnlyWithIssues
