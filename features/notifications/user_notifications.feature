...
Feature: User notifications
  ...
  Scenario: Mentioning users in comments
    # Create task by a third user.
    Given there is a user named "Captain Flint"
    And there is an open task "Ghost Stories" created by "Captain Flint"

    # Mention Batiatus in a comment.
    When I go to the task page for "Ghost Stories"
    ...
    Then I should see "Archibald mentioned you in the comment to the task Ghost Stories"
    And I should only see 1 notification

  Scenario: Notifying creators, and replying to comments (without mentioning their names)
    Given there is an open task "My Favorite Ants" created by "Batiatus"

    # Comment on a task created by Batiatus.
    When I go to the task page for "My Favorite Ants"
    ...
    When I log in as "Batiatus"
    And I go to my notifications page
    Then I should see "Archibald commented on the task My Favorite Ants which you created"
    And I should only see 1 notification
    ...
    And I go to my notifications page
    Then I should see "Batiatus replied to your comment on the task My Favorite Ants"
    And I should only see 1 notification

  Scenario: Send at most one notification to a user for the same comment
    # Make Batiatus the task creator and a participant of the discussion.
    Given there is an open task "My Favorite Ants" created by "Batiatus"
    When I log in as "Batiatus"
    And I go to the task page for "My Favorite Ants"
    And I write a new comment "I'll add more later."
    And I press "Post Comment"
    And I wait for the "success" message

    # Comment on Batiatus' task, mention him, reply to him, in a disussion he is active in.
    When I log in as "Archibald"
    And I go to the task page for "My Favorite Ants"
    And I follow "reply"
    ...
    When I log in as "Batiatus"
    And I go to my notifications page
    Then I should see "Archibald replied to your comment on the task My Favorite Ants"
    And I should only see 1 notification

  Scenario: Do not repeat notifications for any given attached/notifier combination
    Given there is an open task "My Favorite Ants" created by "Batiatus"

    # Edit Batiatus' task twice.
    And I go to the task page for "My Favorite Ants"
    And I follow "Edit"
    And I fill in "task_description" with "Helo @user" followed by the user id of "Batiatus"
    And I press "Save"
    And I wait for the "success" message

    And I follow "Edit"
    And I fill in "task_description" with "Hello @user" followed by the user id of "Batiatus"
    And I press "Save"
    And I wait for the "success" message
    ...
    When I log in as "Batiatus"
    And I go to my notifications page
    Then I should see "Archibald mentioned you in the task My Favorite Ants"
    And I should only see 1 notification


  Scenario: Mentioning users in "things" (task description)
    # Mention Batiatus in the description of a task.
    When I go to the new task form
    And I fill in "task_title" with "Resolve homonyms"
    And I fill in "task_description" with "@user" followed by the user id of "Batiatus"
    And I press "Save"
    And I wait for the "success" message
    ...
    When I log in as "Batiatus"
    And I go to my notifications page
    Then I should see "Archibald mentioned you in the task Resolve homonyms"
    And I should only see 1 notification
