...
describe DuplicateReferencesFinder do
  it "should populate duplicate_references" do
    ...
    target.should_receive(:<=>).with(duplicate).and_return 20
    target.should_receive(:<=>).with(not_a_duplicate).and_return 0
    duplicate.should_not_receive(:<=>).with(duplicate)
    duplicate.should_not_receive(:<=>).with(target)
    duplicate.should_receive(:<=>).with(not_a_duplicate).and_return 0
    not_a_duplicate.should_not_receive(:<=>).with(not_a_duplicate)
    not_a_duplicate.should_receive(:<=>).with(target).and_return 0
    not_a_duplicate.should_receive(:<=>).with(duplicate).and_return 0
    ...
    duplicate_reference.similarity.should == 20
  end
end
