...
describe Ward::BookCitationParser do
  ...
  it "should return nil if it doesn't seem to be a book citation" do
    Ward::BookCitationParser.parse('Science 1:2', false).should be_nil
  end
  ...
  describe "different behavior when possibly embedded (i.e., preceded by a title)" do
    it 'should recognize an unembedded book citation' do
      Ward::BookCitationParser.parse('New York: Wiley, 2 pp.', true).should have_key(:book)
    end
    ...
    it 'should not recognize an embedded book citation when the place has a period' do
      Ward::BookCitationParser.parse('St. Croix: Wiley, 2 pp.', true).should be_false
    end
    it 'should recognize an unembedded book citation when the place has a period' do
      Ward::BookCitationParser.parse('St. Croix: Wiley, 2 pp.', false).should have_key(:book)
    end
    it 'should recognize an embedded book citation when the place has a period and the place is known' do
      ...
      Ward::BookCitationParser.parse('St. Croix: Wiley, 2 pp.', true).should have_key(:book)
    end
  end

  it "should extract the place, pagination and publisher" do
    Ward::BookCitationParser.parse('New York: CSIRO Publications, vii + 70 pp.', false).should == (
  ...
  it "should handle other variations in pagination" do
    ['1 p., 5 maps',
      ...
      Ward::BookCitationParser.parse("New York: Keishu-sha, #{pagination}", false).should ==(
  ...
  it "should handle a publisher with a comma in its name" do
    Ward::BookCitationParser.parse('New York: Little, Brown, vii + 70 pp.', false).should == 
  ...
  it "should handle a publisher with 'i' as a word in its name" do
    Ward::BookCitationParser.parse('New York: Panstwowe Wydawnictwo Rolnicze i Lesne, 55 pp.', false).should ==
...
"Six new weaver ant species from Malaysia: *Camponotus *(*Karavaievia*) *striatipes*, *C.* (*K.*) *melanus*, *C.* (*K.*) *nigripes*, *C.* (*K.*) *belumensis*, *C.* (*K.*) *gentingensis*, and *C.* (*K.*) *micragyne*. Malaysian Journal of Science. Series A. Life Sciences 16:87-105.", false
  ...
  it "should not be fooled by a colon that is part of a pagination note" do
    Ward::BookCitationParser.parse("Journal of Insect Science 7(42), 14 pp. (available online: insectscience.org/7.42).", false).should be_nil
  end

  it "should not be fooled by a volume number" do
    Ward::BookCitationParser.parse("5: Zoologia. Rio de Janeiro: Conselho Nacional de Pesquisas, 603 pp.", true).should be_nil
  end
