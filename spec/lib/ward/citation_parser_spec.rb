...
describe Ward::CitationParser do
  describe "actual parsed data results" do
    ...
    it 'should parse an article citation' do
      Ward::CitationParser.parse('Psyche 1:2').should ==
    ...
    it "should parse an article citation even if there's a space after the colon" do
      Ward::CitationParser.parse('Zootaxa 1929(1): 1-37').should ==
        {:article => {:journal => 'Zootaxa', :series_volume_issue => '1929(1)', :pagination => '1-37'}}
    end

    it 'should parse a book citation' do
      Ward::CitationParser.parse('Melbourne: CSIRO Publications, vii + 70 pp.').should ==
        {:book => {:publisher => {:name => 'CSIRO Publications', :place => 'Melbourne'}, :pagination => 'vii + 70 pp.'}}
    end

    it 'should parse a nested citation' do
      Ward::CitationParser.parse("Pp. 96-98 in: MacKay, W., Lowrie, D., Fisher, A., MacKay, E., Barnes, F., Lowrie, D. The ants of Los Alamos County, New Mexico (Hymenoptera: Formicidae). Pp. 79-131 in: Trager, J. C. (ed.) Advances in myrmecology. Leiden: E. J. Brill, xxvii + 551 pp.").should == {
        :nested => {
    ...
    it 'should parse a completely unknown citation' do
      Ward::CitationParser.parse('A book.').should == {:unknown => 'A book'}
    end

    it "should not parse a completely unknown citation if it's possibly embedded" do
      Ward::CitationParser.parse('A book.', true).should_not be_true
    end

    it "should probably just call this one unknown" do
      Ward::CitationParser.parse("Journal of Insect Science 7(42), 14 pp. (available online: insectscience.org/7.42).").should ==
        {:unknown => "Journal of Insect Science 7(42), 14 pp. (available online: insectscience.org/7.42)"}
    end

    it "should work" do
      Ward::CitationParser.parse("Transactions of the Society for British Entomology 16:93-114,121").should ==
        {:article => {
