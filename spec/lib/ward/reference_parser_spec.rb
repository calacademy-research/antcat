...
describe Ward::ReferenceParser do

  describe 'parsing' do

    it "should not start a citation on a lowercase letter" do
      Factory :place, :name => 'Baltimore'
      Ward::ReferenceParser.parse("Ward, P. 2001. Title with. periods. Baltimore:Wiley, 32 pp.").should == {
        :author_names => ['Ward, P.'],
    ...
    it 'should parse an article reference' do
      Ward::ReferenceParser.parse('Mayer, D.M. 1970. Ants. Psyche 1:2').should == {
    ...
    it 'should parse a book reference' do
      Ward::ReferenceParser.parse('Ward, P.S., Fisher, B.L. 1981. Ants are my life. Melbourne: CSIRO Publications, vii + 70 pp.').should == {
        :author_names => ['Ward, P.S.', 'Fisher, B.L.'],
        :author_names_suffix => nil,
        :year => '1981',
    ...
    it 'should parse a CD-ROM reference' do
      Ward::ReferenceParser.parse('Bolton, B.; Alpert, G.; Ward, P. S.; Nasrecki, P. 2006. Bolton’s Catalogue of ants of the world. Cambridge, Mass.: Harvard University Press, CD-ROM.').should == {
        :author_names => ['Bolton, B.', 'Alpert, G.', 'Ward, P. S.', 'Nasrecki, P.'],
        :author_names_suffix => nil,
        :year => '2006',
    ...
    it 'should parse a completely unknown reference' do
      Ward::ReferenceParser.parse('Bolton, B. 2006. Ants. A book.').should ==
        {:author_names => ['Bolton, B.'], :author_names_suffix => nil, :year => '2006', :title => 'Ants', :unknown => 'A book'}
    end

    it "actually can handle this mess!" do
      Ward::ReferenceParser.parse('Bolton, B. 2006. Taxonomy, phylogeny: Philip Jr., 1904-1983. Series Entomologica (Dordrecht) 33:1-514.').should ==
        {:author_names => ['Bolton, B.'], :author_names_suffix => nil, :year => '2006', :title => 'Taxonomy, phylogeny: Philip Jr., 1904-1983',
    ...
    it "grabs the first sentence without a period as the journal name" do
      Ward::ReferenceParser.parse("Ward, P. 1980. Ameisen aus Sao Paulo (Brasilien), Paraguay etc. gesammelt von Prof. Herm. v. Ihering, Dr. Lutz, Dr. Fiebrig, etc. Verhandlungen der Kaiserlich-Königlichen Zoologisch-Botanischen Gesellschaft in Wien 58:340-418").should == {
        :author_names => ['Ward, P.'],
        :author_names_suffix => nil,
        :year => '1980',
