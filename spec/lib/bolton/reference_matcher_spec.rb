...
describe Bolton::ReferenceMatcher do
  ...
  #describe "matching Bolton's references against Ward's" do
    ...
    #it 'should give less confidence than 1.0 if the title is slightly different' do
      #@bolton.update_attributes :title => @ward.title + 'x'
      #@matcher.find_matches_for @bolton
      #@bolton.matches.count.should == 1
      #@bolton.matches.first.confidence.should be < 1.0
    #end
