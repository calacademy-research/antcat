  ...
  describe "Genera header" do
    describe "Genera header" do
      it "should be recognized" do
        @grammar.parse(%{Genera of Aneuretini}).value.should == {:type => :genera_header}
      end
      it "should be recognized when there's only one genus" do
        @grammar.parse(%{Genus of Dolichoderini}).value.should == {:type => :genera_header}
      end
      it "should be recognized when it's this one weird tribe name" do
        @grammar.parse(%{Genus of Platythyreini}, :root => :genera_header).value.should == {:type => :genera_header}
      end
    end

    describe "Genera incertae sedis header" do
      it "should be recognized" do
        @grammar.parse(%{Genera <i>incertae sedis</i> in ANEURETINAE}).value.should == {:type => :genera_incertae_sedis_header}
      end
      it "should be recognized when there's only one genus" do
        @grammar.parse(%{Genus <i>incertae sedis</i> in ANEURETINAE}).value.should == {:type => :genera_incertae_sedis_header}
      end
      it "should be recognized when extinct" do
        @grammar.parse(%{Genera (extinct) <i>incertae sedis</i> in DOLICHODERINAE}).value.should == {:type => :genera_incertae_sedis_header}
      end
      it "should be recognized when Hong's" do
        @grammar.parse(%{Genera of Hong (2002), <i>incertae sedis</i> in FORMICINAE}).value.should == {:type => :genera_of_hong_header}
      end
      it "should be recognized in poneroid subfamilies" do
        @grammar.parse('Genera <i>incertae sedis</i> in poneroid subfamilies').value.should == {
  ...
  describe "Genus headers" do
    describe "Genus header" do
      it "should recognize a genus header" do
        @grammar.parse(%{Genus <i>ATTA</i>}).value.should == {:type => :genus_header, :name => 'Atta'}
      end
      it "should recognize the beginning of a fossil genus" do
        @grammar.parse(%{Genus *<i>ANEURETELLUS</i>}).value.should == {:type => :genus_header, :name => 'Aneuretellus', :fossil => true}
      end

      it "should handle it when there's a subfamily at the end" do
        @grammar.parse(%{Genus *<i>YPRESIOMYRMA</i> [Myrmeciinae]}).value.should ==
    ...
    describe "Genus nomen nudum header" do
      it "should handle it" do
        @grammar.parse(%{<i>ANCYLOGNATHUS</i> [<i>nomen nudum</i>]}).value.should == {:type => :genus_nomen_nudum_header, :name => 'Ancylognathus'}
      end
      it "should handle a fossil nomen nudum header" do
        @grammar.parse(%{*<i>DOLICHOFORMICA</i> [<i>nomen nudum</i>]}).value.should ==
  ...
  describe "Genus record" do
    describe "Type species" do
      it "should handle a type-species by subsequent designation" do
        @grammar.parse('Type-species: <i>Bothriomyrmex myops</i>, by subsequent designation of Donisthorpe, 1944e: 102.', :root => :type_species).value.should == {
          :type_species => {
      ...
      it "should handle a type-species that it doesn't understand" do
        @grammar.parse('Type-species: none subsequent', :root => :type_species).value.should == {
    ...
    it "should recognize a genus headline" do
      @grammar.parse(%{<i>Odontomyrmex</i> André, 1905: 207. Type-species: <i>Odontomyrmex quadridentatus</i>, by monotypy.}).value.should == {
        :type => :genus_headline,
    ...
    it "should handle a genus headline where the genus is nomen nudum as well as the type-species" do
      @grammar.parse('*<i>Dolichoformica</i> Grimaldi & Engel, 2005: 446 (in table), <i>nomen nudum</i>. Type-species: *<i>Dolichoformica helferi</i>, <i>nomen nudum</i>.').value.should == {
        :type => :genus_headline,
    ...
    it "should recognize a genus headline with nomen nudum" do
      @grammar.parse(
%{*<i>Dolichoformica</i> Grimaldi & Engel, 2005: 446 (in table), <i>nomen nudum</i>.}).value.should == {
        :type => :genus_headline,
        :genus_name => 'Dolichoformica',
        :authorship => [{:author_names => ['Grimaldi', 'Engel'], :year => '2005', :pages => '446 (in table)'}],
        :nomen_nudum => true,
    ...
    it "should handle bracketed phrase after genus nomen nudum" do
      @grammar.parse('<i>Myrmegis</i> Rafinesque, 1815: 124, <i>nomen nudum</i>. [Brown, 1973b: 182, places <i>Myrmegis</i> as a junior synonym of <i>Atta</i>, because the entry in Rafinesque reads, "6. <i>Myrmegis</i> R. <i>Atta</i> Latr."]').value
    end

    it "should recognize a fossil genus headline with a note" do
      @grammar.parse(%{*<i>Calyptites</i> Scudder, 1877b: 270 [as member of family Braconidae]. Type-species: *<i>Calyptites antediluvianum</i>, by monotypy.}).value.should == {
        :type => :genus_headline,
    ...
    it "should recognize a subgenus" do
      @grammar.parse(%{<i>Hypochira</i> Buckley, 1866: 169 [as subgenus of <i>Formica</i>]. Type-species: <i>Formica (Hypochira) subspinosa</i>, by monotypy.}).value.should == {
        :type => :genus_headline,
    ...
    it "should recognize a type-species that's a junior synonym" do
      @grammar.parse(%{*<i>Eoformica</i> Cockerell, 1921: 38. Type-species: *<i>Eoformica eocenica</i> (junior synonym of *<i>Eoformica pingue</i>), by monotypy.}).value.should == {
        :type => :genus_headline,
    ...
    it "should recognize a type-species by original designation" do
      @grammar.parse(%{*<i>Gerontoformica</i> Nel & Perrault, in Nel, Perrault, Perrichot & Néraudeau, 2004: 24. Type-species: *<i>Gerontoformica cretacica</i>, by original designation.}).value.should == {
        :type => :genus_headline,
    ...
    it "should recognize the 'genus headline' that actually describes a collective group name" do
      @grammar.parse(%{*<i>Myrmeciites</i> Archibald, Cover & Moreau, 2006: 500. [Collective group name.]}).value.should == {
        :type => :genus_headline,
    ...
    it "should recognize an unnecessary replacement name" do
      @grammar.parse(%{<i>Baroniurbania</i> Pagliano & Scaramozzino, 1990: 4. Unnecessary replacement name for <i>Acantholepis</i> Mayr (junior homonym).}).value.should == {
        :type => :genus_headline,
    ...
    it "should recognize an unnecessary replacement name for sensu name" do
      @grammar.parse('<i>Parasima</i> Donisthorpe, 1948d: 592 [as subgenus of <i>Tetraponera</i>]. [Unnecessary replacement name for <i>Sima</i> in the sense of Emery, 1921f: 23.]').value.should == {
        :type => :genus_headline,
        :genus_name => 'Parasima',
    ...
    it "should recognize an unjustified emendation" do
      @grammar.parse('<i>Ceratopachys</i> Schulz, W.A. 1906: 155, unjustified emendation of <i>Cerapachys</i>.').value.should == {
        :type => :genus_headline,
    ...
    it "should recognize a subsequent unjustified emendation" do
      @grammar.parse('<i>Vollenhovenia</i> Dalla Torre, 1893: 61, unjustified subsequent emendation of <i>Vollenhovia</i>.').value.should == {
        :type => :genus_headline,
  ...
  describe "Homonym replaced by genus header" do
    it "should be recognized" do
      @grammar.parse(%{Homonym replaced by *<i>PROMYRMICIUM</i>}).value.should == {:type => :homonym_replaced_by_genus_header}
    end
    it "should be recognized" do
      @grammar.parse(%{Homonym replaced by <i>STIGMACROS</i>}).value.should == {:type => :homonym_replaced_by_genus_header}
    end
    it "should be recognized as :other when a homonym-replaced-by in a synonym" do
      @grammar.parse(%{Homonym replaced by <i>Karawajewella</i>}).value.should == {:type => :homonym_replaced_by_genus_header}
    end
  end

  describe "Junior synonym headers" do
    it "should recognize a header for the group of synonyms" do
      @grammar.parse(%{Junior synonyms of <i>ANEURETUS</i>}).value.should == {:type => :junior_synonyms_of_genus_header}
    end
    it "should recognize a header for the group of synonyms when there's only one" do
      @grammar.parse(%{Junior synonym of <i>ANEURETUS</i>}).value.should == {:type => :junior_synonyms_of_genus_header}
    end
    it "should recognize a header for the group of synonyms when there's a period after the closing tags" do
      @grammar.parse(%{Junior synonyms of <i>ACROPYGA</i>.}).value.should == {:type => :junior_synonyms_of_genus_header}
    end
    it "should be recognized when they are of a fossil" do
      @grammar.parse(%{Junior synonyms of *<i>ARCHIMYRMEX</i>}).value.should == {:type => :junior_synonyms_of_genus_header}
    end
  end

  describe "Genus references header" do
    it "should handle a regular header" do
      @grammar.parse(%{Genus <i>Sphinctomyrmex</i> references}).value.should == {:type => :genus_references_header}
    end
    it "should handle a header without a name" do
      @grammar.parse(%{Genus references}).value.should == {:type => :genus_references_header}
    end
    it "should handle a 'see above' header" do
      @grammar.parse(%{Genus <i>Myrmoteras</i> references: see above.}).value.should == {:type => :genus_references_see_under}
    end
    it "should handle a 'see above' header without the genus name" do
      @grammar.parse(%{Genus references: see above.}).value.should == {:type => :genus_references_see_under}
    end
    it "should handle a 'see above' header without the genus name" do
      @grammar.parse(%{Genus references: see under Aneuretini, above.}).value.should == {
  ...
  describe "Taxonomic history items" do
    describe "anything we can't otherwise deal with, for now" do
      it "should parse a bracketed phrase as text" do
        @grammar.parse(%{[Type-species not <i>Tapinoma instabilis</i>]}, :root => :genus_taxonomic_history_item).value.should ==
    ...
    it "should handle this one that looks like a headerline" do
      @grammar.parse('*<i>Ponerites</i> Dlussky, 1981b: 67 [collective group name].', :root => :genus_taxonomic_history_item).value.should == {
        :type => :genus_taxonomic_history_item,
    ...
    describe "'in family'" do
      it "should parse 'in family'" do
        @grammar.parse(%{<i>Condylodon</i> in family Mutillidae: Swainson & Shuckard, 1840: 173.}, :root => :genus_taxonomic_history_item).value.should ==
          {:type => :genus_taxonomic_history_item,
      ...
      it "should parse 'in <family>, <subfamily>" do
        @grammar.parse('*<i>Promyrmicium</i> in Formicidae, Myrmicinae: Baroni Urbani, 1971b: 362; Bolton, 1994: 106 (anachronism).', :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
      ...
      it "should parse 'as subgenus of" do
        @grammar.parse('<i>Chronoxenus</i> as subgenus of <i>Bothriomyrmex</i>: Santschi, 1919i: 202.', :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
      ...
      it "should parse 'in <subfamily>, <tribe>, <subtribe>" do
        @grammar.parse('<i>Arnoldius</i> in Dolichoderinae, Iridomyrmecini, Bothriomyrmecina: Dubovikov, 2005a: 93.', :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
      ...
      it "should parse 'in subfamily, fossil tribe'" do
        @grammar.parse('*<i>Pityomyrmex</i> in Dolichoderinae, *Pityomyrmecini: Wheeler, W.M. 1915h: 98.', :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
      ...
      it "should parse special case of Myrmiciidae (Symphyta)" do
        @grammar.parse(%{*<i>Myrmicium</i> in *Myrmiciidae (Symphyta): Maa, 1949: 17; Rasnitsyn, 1969: 17; Jarzembowski, 1993: 179.}, :root => :genus_taxonomic_history_item).value.should ==
          {:type => :genus_taxonomic_history_item,
      ...
      it "should handle a fossil, and more than one reference" do
        @grammar.parse(%{*<i>Calyptites</i> in Braconidae: Scudder, 1891: 629; Donisthorpe, 1943f: 629.}, :root => :genus_taxonomic_history_item).value.should ==
          {:type => :genus_taxonomic_history_item,
      ...
      it "should parse 'in family, tribe'" do
        @grammar.parse(%{*<i>Eoformica</i> in Formicinae, Oecophyllini: Donisthorpe, 1943f: 643.}, :root => :genus_taxonomic_history_item).value.should ==
          {:type => :genus_taxonomic_history_item,
      ...
      describe "'dubiously'" do
        it "should parse 'not in family, dubiously family'" do
          @grammar.parse(%{<i>Condylodon</i> not in Pseudomyrmecinae, dubiously Ponerinae: Ward, 1990: 471.}, :root => :genus_taxonomic_history_item).value.should == {
            :type => :genus_taxonomic_history_item,
            :genus => {:genus_name => "Condylodon"},
            :in => [
        ...
        it "should parse 'dubiously family'" do
          @grammar.parse(%{*<i>Cretacoformica</i> dubiously Formicidae: Bolton, 1995b: 25, 166.}, :root => :genus_taxonomic_history_item).value.should == {
            :type => :genus_taxonomic_history_item,
      ...
      it "should parse 'in <order>" do
        @grammar.parse(%{*<i>Palaeomyrmex</i> in Homoptera: Handlirsch, 1906: 507.}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
      ...
      it "should handle bracketed note at end" do
        @grammar.parse(%{<i>Bothriomyrmex</i> in Formicinae: André, 1881b: 64 [Formicidae].}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
    ...
    describe "'incertae sedis in'" do
      it "should parse 'incertae sedis in family' (with parenthetical note)" do
        @grammar.parse(%{*<i>Calyptites</i> <i>incertae sedis</i> in Formicidae: Carpenter, 1930: 21; Brown, 1973b: 179; Bolton, 1995b: 23, 83 (family unresolved).}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
          :genus => {:genus_name => "Calyptites", :fossil => true},
      ...
      it "should parse 'incertae sedis in order, order'" do
        @grammar.parse(%{*<i>Cariridris</i> <i>incertae sedis</i> in Hymenoptera, Aculeata: Grimaldi, Agosti & Carpenter, 1997: 7.}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
    ...
    describe "'also described as new'" do
      it "should handle this" do
        @grammar.parse('[<i>Aneuretus</i> also described as new by Emery, 1893f: 241.]', :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
    ...
    describe "nomina nuda" do
      it "should handle it" do
        @grammar.parse("[*<i>Paraneuretus</i> Wheeler, W.M. 1908g: 413; <i>nomina nuda</i>.]", :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
          :genus => {
    ...
    describe "'junior synonym of'" do
      it "should parse 'junior synonym of'" do
        @grammar.parse(%{<i>Condylodon</i> as junior synonym of <i>Pseudomyrma</i>: Dalla Torre, 1893: 55.}, :root => :genus_taxonomic_history_item).value.should ==
          {:type => :genus_taxonomic_history_item,
           :genus => {:genus_name => 'Condylodon'},
      ...
      it "should parse 'as questionable junior synonym of'" do
        @grammar.parse(%{<i>Hypochira</i> as questionable junior synonym of <i>Dolichoderus</i>: Emery, 1895c: 338; Emery, 1925b: 271.}, :root => :genus_taxonomic_history_item).value.should ==
          {:type => :genus_taxonomic_history_item,
    ...
    it "should parse 'excluded from family'" do
      @grammar.parse(%{*<i>Cretacoformica</i> excluded from Formicidae: Naumann, 1993: 355.}, :root => :genus_taxonomic_history_item).value.should ==
        {:type => :genus_taxonomic_history_item,
          :genus => {:genus_name => 'Cretacoformica', :fossil => true},
    ...
    it "should parse 'as genus'" do
      @grammar.parse(%{*<i>Myrmicium</i> as genus: Westwood, 1854: 396.}, :root => :genus_taxonomic_history_item).value.should ==
        {:type => :genus_taxonomic_history_item,
          :genus => {:genus_name => 'Myrmicium', :fossil => true},
          :as_genus => true,
    ...
    describe "Incorrect subsequent spelling" do
      it "should handle an incorrect subsequent spelling" do
        @grammar.parse(%{*<i>Myrmicium</i> in Myrmicinae: Dalla Torre, 1893: 108 (as *<i>Myrmecium</i>, incorrect subsequent spelling); Bolton, 1995b: 37 (anachronism).}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
          :genus => {:genus_name => 'Myrmicium', :fossil => true},
      ...
      it "should handle another form of incorrect subsequent spelling" do
        @grammar.parse(%{<i>Bregmatomyrmex</i> [incorrect subsequent spelling] <i>incertae sedis</i> in Formicidae: Wheeler, G.C. & Wheeler, J. 1985: 259.}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
    ...
    describe "Bracketed items" do
      it "should handle the bracketed 'replacement name for' note" do
        @grammar.parse(%{[Replacement name for *<i>Myrmicium</i> Heer, 1870: 78; junior homonym of *<i>Myrmicium</i> Westwood, 1854: 396.]}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
      ...
      it "should handle the bracketed 'junior homonym of' note" do
        @grammar.parse(%{[Junior homonym of *<i>Myrmicium</i> Westwood, 1854: 396 (*Pseudosiricidae).]}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
          :junior_homonym_of => {
      ...
      it "A genus name, and then a class name in parentheses" do
        @grammar.parse(%{[<i>Diabolus</i> Karavaiev junior homonym of <i>Diabolus</i> Gray, J.E. 1841: 400 (Mammalia).]}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
      ...
      it "should handle the bracketed item about Formicites" do
        @grammar.parse(%{[*<i>Formicites</i> (collective group name) material absorbed into *<i>Eoformica</i>: Dlussky & Rasnitsyn, 2002: 424.]}, :root => :genus_taxonomic_history_item).value.should == {
          :type => :genus_taxonomic_history_item,
          :material_absorbed => {
            :from => {
