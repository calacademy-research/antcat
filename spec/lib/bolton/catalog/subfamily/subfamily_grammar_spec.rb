  ...
  describe "Lists" do
    ...
    describe "Parsing a group of list names" do
      it "should recognize one name" do
        Bolton::Catalog::Subfamily::Grammar.parse("*<i>Myrmeciites</i>.", :root => :list_names).value_with_reference_text_removed.should == [{:name => 'Myrmeciites', :fossil => true}]
      end
      it "should recognize more than one name" do
        Bolton::Catalog::Subfamily::Grammar.parse(%{*<i>Myrmeciites</i>, <i>Petropone</i>.}, :root => :list_names).value_with_reference_text_removed.should ==
