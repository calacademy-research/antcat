...
describe Bolton::Catalog::Subfamily::TribeGrammar do
  ...
  describe "Tribe header" do
    it "should be recognized" do
      @grammar.parse(%{Tribe MYRMECIINI}).value.should == {:type => :tribe_header, :name => 'Myrmeciini'}
    end
    it "should be recognized when it's extinct" do
      @grammar.parse(%{Tribe *MIOMYRMECINI}).value.should == {:type => :tribe_header, :name => 'Miomyrmecini', :fossil => true}
    end
    it "should be recognized when the asterisk is in a different place" do
      @grammar.parse(%{Tribe *PITYOMYRMECINI}).value.should == {:type => :tribe_header, :name => 'Pityomyrmecini', :fossil => true}
    end
  end

  describe "Junior synonyms of tribe header" do
    it "should be recognized" do
      @grammar.parse(%{Junior synonym of ECTATOMMINI}).value.should == {:type => :junior_synonyms_of_tribe_header}
    end
    it "should be recognized when plural" do
      @grammar.parse(%{Junior synonyms of LEPTOMYRMECINI}).value.should == {:type => :junior_synonyms_of_tribe_header}
    end
  end

  describe "Genera incertae sedis in tribe" do
    it "should be recognized" do
      @grammar.parse(%{Genus <i>incertae sedis</i> in Heteroponerini}).value.should == {:type => :genera_incertae_sedis_in_tribe_header}
    end
  end

  describe "Tribes incertae sedis in subfamily header" do
    it "should be recognized" do
      @grammar.parse(%{Tribes (extinct) <i>incertae sedis</i> in DOLICHODERINAE}).value.should == {:type => :tribes_incertae_sedis_header}
    end
  end

  describe "Taxonomic history items" do
    it "should parse 'tribe of'" do
      @grammar.parse(%{Aneuretini as tribe of Dolichoderinae: Emery, 1913a: 6; all subsequent authors.}, :root => :tribe_taxonomic_history_item).value.should ==
    ...
    it "should parse 'subtribe of'" do
      @grammar.parse(%{Stictoponerini as subtribe of Aneuretini: Arnol'di, 1930d: 161.}, :root => :tribe_taxonomic_history_item).value.should ==
        {:type => :tribe_taxonomic_history_item,
    ...
    it "should parse a subtribe 'as subtribe of'" do
      @grammar.parse(%{Bothriomyrmecina as subtribe of Iridomyrmecini: Dubovikov, 2005a: 92.}, :root => :tribe_taxonomic_history_item).value.should ==
        {:type => :tribe_taxonomic_history_item,
    ...
    it "should parse 'junior synonym of'" do
      @grammar.parse(%{*Pityomyrmecini as junior synonym of Dolichoderinae: Shattuck, 1992c: 5.}, :root => :tribe_taxonomic_history_item).value.should ==
        {:type => :tribe_taxonomic_history_item,
    ...
    it "should parse 'junior synonym of' a tribe" do
      @grammar.parse('Anonychomyrmini as junior synonym of Leptomyrmecini: Ward, Brady, <i>et al.</i> 2010: 361.', :root => :tribe_taxonomic_history_item).value.should == {:type => :tribe_taxonomic_history_item,
    ...
    it "should parse 'incertae sedis in <subfamily>'" do
      @grammar.parse(%{*Pityomyrmecini <i>incertae sedis</i> in Dolichoderinae: Ward, Brady, <i>et al.</i> 2010: 362.}, :root => :tribe_taxonomic_history_item).value.should == {
        :type => :tribe_taxonomic_history_item,
        :tribe => {:tribe_name => "Pityomyrmecini", :fossil => true},
  ...
  describe "Ichnotaxon" do
    it "list should be recognized" do
      @grammar.parse('Ichnotaxon: *<i>Attaichnus</i>.').value.should == {
        :type => :ichnotaxa_list, :genus => {:genus_name => 'Attaichnus', :fossil => true}
      }
    end

    it "items should be recognized" do
      @grammar.parse('Ichnotaxon attached to Attini', :root => :ichnotaxa_header).value.should == {
  ...
  describe "References header" do
    ...
    it "should handle references for the tribe when the name is left out" do
      @grammar.parse('Tribe references').value.should == {
        :type => :references_section_header,
        :taxa => {:tribe => true}
      }
    end
    it "should handle a see also references header" do
      @grammar.parse('See also general references under PONERINAE.').value.should == {
    ...
    it "should handle references for a see above tribe" do
      @grammar.parse('Tribe Ecitonini references: see above').value.should == {
    ...
    it "should handle references for a see above tribe, not named" do
      @grammar.parse('Tribe references: see above').value.should == {
        :type => :see_under_references_section_header,
        :taxa => :tribe
      }
    end
    ...
    it "should handle references for the tribe and a genus" do
      @grammar.parse('Tribe and genus <i>Myrmoteras</i> references').value.should == {
        :type => :references_section_header,
        :taxa => {:genus_name => 'Myrmoteras'}
      }
    end
    it "should handle references for the subfamily and tribe" do
      @grammar.parse('Subfamily and tribe Pseudomyrmecini references').value.should == {
        :type => :references_section_header,
        :taxa => {:subfamily => true, :tribe_name => 'Pseudomyrmecini'}
      }
    end
    it "should handle a header that also includes a reference" do
      @grammar.parse('Tribe references: see under genera; Bolton, 2003: 30.').value.should == {
        :type => :references_section_header,
        :texts => [
          :text => [
