...
describe Bolton::Catalog::Grammar do
  ...
  describe "Taxon authorship" do
    ...
    it "can be a single author name followed by 'above'" do
      @grammar.parse('Cuezzo, above', :root => :authorship).value.should == [{
        :author_names => ['Cuezzo']
      }]
    end
    it "can be an author name with year, page, and 'above" do
      @grammar.parse('Smith, F. 1858b: 13, above', :root => :authorship).value.should == [{
        :author_names => ['Smith, F.'], :year => '1858b', :pages => '13'
      }]
    end
    it "can be a single author name plus year" do
      @grammar.parse('Cuezzo, 1922', :root => :authorship).value.should == [{
        :author_names => ['Cuezzo'], :year => '1922'
      }]
    end
    it "can be a single author name plus year plus pages" do
      @grammar.parse('Cuezzo, 1922: 1', :root => :authorship).value.should == [{
        :author_names => ['Cuezzo'], :year => '1922', :pages => '1'
      }]
    end
    it "can be a single author name without year or pages but with 'above'" do
      @grammar.parse('Forel (above)', :root => :authorship).value.should == [{
        :author_names => ['Forel']
      }]
    end
    it "can be nested" do
      @grammar.parse('Gmelin, in Linnaeus, 1790: 2804 (w.)', :root => :authorship).value.should == [{
        :author_names => ['Gmelin'],
        :pages => '2804',
  ...
  describe "References" do
    it "can handle multiple references for a single author" do
      @grammar.parse('Emery, 1908a: 165, 1908c: 305, 1908: 437', :root => :references).value.should ==
        {:references => [
          {:author_names => ['Emery'],
           :publications => [
    ...
    it "can handle a colon after the author" do
      @grammar.parse('Gonçalves: 1961: 153', :root => :references).value.should == {:references => [
        {:author_names => ['Gonçalves'], :year => '1961', :pages => '153'}
      ]}
    end
    it "can have forms" do
      @grammar.parse('Cuezzo, 1922: 32 (w.q.m.)', :root => :references).value.should == {:references => [
        {:author_names => ['Cuezzo'], :year => '1922', :pages => '32', :forms => 'w.q.m.'}
      ]}
    end
    it "can be a single author name plus year plus pages" do
      @grammar.parse('Cuezzo, 1922: 1', :root => :references).value.should == {:references => [
        {:author_names => ['Cuezzo'], :year => '1922', :pages => '1'}
      ]}
    end
    ...
    it "should parse multiple references" do
      @grammar.parse('Cuezzo, 1900: 36; Bolton, 2000: 23', :root => :references).value.should == {:references => [
    ...
    it "should parse multiple references with forms" do
      @grammar.parse('Wheeler, G.C. & Wheeler, J. 1966: 730 (l.); Cuezzo, 2000: 231 (q.m.)',
                      :root => :references).value.should == {:references => [
    ...
    it "should parse this reference with complicated forms" do
      @grammar.parse('Eguchi, 2008: 238 (s.w.ergatoid q.)', :root => :reference).value.should == 
  ...
  describe "Author names" do
    it "should handle 'do'" do
      @grammar.parse('Fernández, Delabie & do Nascimento', :root => :ref_author_names).value.should == ['Fernández', 'Delabie', 'do Nascimento'] 
    end
    it "should handle this name" do
      name = 'Lepeletier de Saint-Fargeau'
      @grammar.parse(name, :root => :ref_author_names).value.should == [name]
    end
    it "should parse Rossi de Garcia" do
      @grammar.parse('Rossi de Garcia', :root => :ref_author_names).value.should == ['Rossi de Garcia']
    end
    it "should parse an author name starting with 'de'" do
      @grammar.parse('de Romand', :root => :ref_author_names).value.should == ['de Romand']
    end
    it "should parse an author name without a space after the last name" do
      @grammar.parse('Smith,F.', :root => :ref_author_names).value.should == ['Smith,F.']
    end
    it "should parse an author name with an umlaut" do
      @grammar.parse('Özdikmen', :root => :ref_author_names).value.should == ['Özdikmen']
    end
    it "should parse a single author name" do
      @grammar.parse('Cuezzo', :root => :ref_author_names).value.should == ['Cuezzo']
    end
    it "should parse two author names" do
      @grammar.parse('Bolton & Fisher', :root => :ref_author_names).value.should == ['Bolton', 'Fisher']
    end
    ...
    it "should parse two author names with initials, including one with two" do
      @grammar.parse('Wheeler, G.C. & Wheeler, J.', :root => :ref_author_names).value.should == ['Wheeler, G.C.', 'Wheeler, J.']
    end
    it "should parse author names with particles" do
      @grammar.parse('De Groot & Le Torquet', :root => :ref_author_names).value.should == ['De Groot', 'Le Torquet']
    end
    it "should parse three authors with et al." do
      @grammar.parse('Imai, Baroni Urbani, Kubota, <i>et al.</i>', :root => :ref_author_names).value.should == ['Imai', 'Baroni Urbani', 'Kubota', '<i>et al.</i>']
    end
    it "should parse three authors" do
      @grammar.parse(%{Trager, MacGown & Trager}, :root => :ref_author_names).value.should == ['Trager', 'MacGown', 'Trager']
    end
    it "should parse an author with two last names" do
      @grammar.parse('Dalla Torre', :root => :ref_author_names).value.should == ['Dalla Torre']
    end
    it "should parse an author list with et al." do
      @grammar.parse('Dalla Torre, <i>et al.</i>', :root => :ref_author_names).value.should == ['Dalla Torre', '<i>et al.</i>']
    end
  ...
  describe "Forms" do
    ...
    it "should parse forms" do
      @grammar.parse('(w.q.)', :root => :ref_forms).value.should == 'w.q.'
    end
    it "should handle quoted phrases" do
      @grammar.parse(%{(s.q. "dwarf q." m.)}, :root => :ref_forms).value.should == 's.q. "dwarf q." m.'
    end
    it "should parse one form it doesn't know about, as long as it's an initial" do
      @grammar.parse('(l.)', :root => :ref_forms).value.should == 'l.'
    end
    it "should parse forms it doesn't know about, as long as they're initials" do
      @grammar.parse('(a.b.)', :root => :ref_forms).value.should == 'a.b.'
    end
    ...
    it "should parse one initial followed by ?" do
      ...
      @grammar.parse('(w?)', :root => :ref_forms).value.should == 'w?'
    end
    it "should parse certain phrases explicitly" do
      for form in ['caste?', 'wing', 'no sex or caste given', 'no caste mentioned', 'no caste', 'no caste or sex given', 'no caste given']
        @grammar.parse("(#{form})", :root => :ref_forms).value.should == form
      end
    end
    ...
    it "should parse one phrase without parentheses" do
      @grammar.parse('no caste given', :root => :ref_forms).value.should == 'no caste given'
    end
  ...
  describe "Pages" do
    ...
    it "should parse figure range with letters" do
      @grammar.parse('figs. 4a-b', :root => :ref_pages).value.should == 'figs. 4a-b'
    end
    it "should parse uppercase figure range" do
      @grammar.parse('figs. 39B-C', :root => :ref_pages).value.should == 'figs. 39B-C'
    end
    it "should parse uppercase figure range" do
      @grammar.parse('figs. A-D', :root => :ref_pages).value.should == 'figs. A-D'
    end
    it "should parse spaces around the hyphen" do
      @grammar.parse('figs. 1 - 4', :root => :ref_pages).value.should == 'figs. 1 - 4'
    end
    it "should parse pages" do
      @grammar.parse('227', :root => :ref_pages).value.should == '227'
    end
    it "should parse pages with figures" do
      @grammar.parse('227, figs. 12, 35', :root => :ref_pages).value.should == '227, figs. 12, 35'
    end
    it "should parse pages that merely indicate the figures on a page" do
      @grammar.parse('193, figs.', :root => :ref_pages).value.should == '193, figs.'
    end
    it "should parse pages with three figures" do
      @grammar.parse('227, figs. 12, 35, 104', :root => :ref_pages).value.should == '227, figs. 12, 35, 104'
    end
    it "should parse pages with one figure" do
      @grammar.parse('227, fig. 12', :root => :ref_pages).value.should == '227, fig. 12'
    end
    it "should parse pages with a range of pages" do
      @grammar.parse('227-228', :root => :ref_pages).value.should == '227-228'
    end
    it "should parse pages with a range of figures" do
      @grammar.parse('227, fig. 12-3', :root => :ref_pages).value.should == '227, fig. 12-3'
    end
    it "should parse pages with (in key)" do
      @grammar.parse('227 (in key)', :root => :ref_pages).value.should == '227 (in key)'
    end
    it "should parse pages with (in key) even without space" do
      @grammar.parse('227(in key)', :root => :ref_pages).value.should == '227(in key)'
    end
    it "should parse pages with (diagnosis in key)" do
      @grammar.parse('227 (diagnosis in key)', :root => :ref_pages).value.should == '227 (diagnosis in key)'
    end
    it "should parse pages with (footnote)" do
      @grammar.parse('227 (footnote)', :root => :ref_pages).value.should == '227 (footnote)'
    end
    it "should parse pages with (in table), followed by another page" do
      @grammar.parse('227 (in table), 6', :root => :ref_pages).value.should == '227 (in table), 6'
    end
    it "should parse pages with (in table)" do
      @grammar.parse('227 (in table)', :root => :ref_pages).value.should == '227 (in table)'
    end
    it "should parse pages with Roman numerals" do
      @grammar.parse('xxxix', :root => :ref_pages).value.should == 'xxxix'
    end
    it "should parse pages with plates and figure numbers" do
      @grammar.parse('18, pl. 1, fig. 19', :root => :ref_pages).value.should == '18, pl. 1, fig. 19'
    end
    ...
    it "should parse pages with plates and figure numbers separated by semicolons" do
      @grammar.parse('215, pl. 15, figs. 9-13; pl. 4, fig. 3', :root => :ref_pages).value.should == '215, pl. 15, figs. 9-13; pl. 4, fig. 3'
    end
    it "should handle figures with letters" do
      @grammar.parse(%{7, figs. 4b, 5b, 6b}, :root => :ref_pages).value.should == '7, figs. 4b, 5b, 6b'
    end
    it "should handle 'no.'" do
      @grammar.parse('no. 1651', :root => :ref_pages).value.should == 'no. 1651'
    end
    it "should handle 'page?'" do
      @grammar.parse('page?', :root => :ref_pages).value.should == 'page?'
    end
  ...
  describe "References with notes" do
    it "should handle a simple phrase" do
      @grammar.parse(%{Heer, 1870: 78 (family unresolved)}, :root => :reference).value.should == {
        :author_names => ['Heer'], :year => '1870', :pages => '78',
        :notes => [[{:phrase => 'family unresolved'}]]
      }
    end
    it "should handle multiple phrase with a taxon name" do
      @grammar.parse(%{Heer, 1870: 78 (as *<i>Myrmecium</i>, incorrect subsequent spelling)}, :root => :reference).value.should == {
        :author_names => ['Heer'], :year => '1870', :pages => '78',
        :notes => [[
          {:phrase => 'as', :delimiter => ' '},
    ...
    it "should handle a bracketed note that begins with a lowercase character" do
      @grammar.parse(%{Emery, 1909c: 355 [as "group" of Ponerinae]}, :root => :reference).value.should == {
    ...
    it "should handle both parenthesized and bracketed notes for the same reference" do
      @grammar.parse('Kusnezov, 1964: 62 (alternatively spelled Palaeoattini, p. 146) [as subdivision of tribe Attini]', :root => :reference).value.should == {
    ...
    it "should handle multiple references with notes" do
      @grammar.parse(%{Heer, 1870: 78 (as *<i>Myrmecium</i>, incorrect subsequent spelling); Heer, 1870: 78 (family unresolved)}, :root => :references).value.should == {
        :references => [
          {:author_names => ['Heer'], :year => '1870', :pages => '78',
            :notes => [[
              {:phrase => 'as', :delimiter => ' '},
