  ...
  describe "Taxonomic history references" do
    ...
    describe 'subsequent authors' do
      it "should handle 'subsequent authors and <reference>'" do
        @grammar.parse(%{Emery, 1913a: 27; subsequent authors and Jaffe, 1993: 9}, :root => :history_references).value_with_matched_text_removed.should == {:references => [
      ...
      it "should handle 'all subsequent authors and <reference>'" do
        @grammar.parse(%{Emery, 1913a: 27; all subsequent authors and Jaffe, 1993: 9}, :root => :history_references).value_with_matched_text_removed.should == {:references => [
            {:author_names => ['Emery'], :year => '1913a', :pages => '27'},
            {:subsequent_authors => 'all subsequent authors and Jaffe, 1993: 9'}
          ]}
      end
      it "should handle 'all subsequent authors'" do
        @grammar.parse(%{Emery, 1913a: 27; all subsequent authors}, :root => :history_references).value_with_matched_text_removed.should == {
          :references => [
            {:author_names => ['Emery'], :year => '1913a', :pages => '27'},
      ...
      it "should handle 'all subsequent authors to the following'" do
        @grammar.parse(%{Emery, 1913a: 27; all subsequent authors to the following}, :root => :history_references).value_with_matched_text_removed.should == {
          :references => [
            {:author_names => ['Emery'], :year => '1913a', :pages => '27'},
    ...
    it "should handle bracketed notes" do
      @grammar.parse(%{André, 1881b: 64 [Formicidae]}, :root => :history_reference).value_with_matched_text_removed.should == {
