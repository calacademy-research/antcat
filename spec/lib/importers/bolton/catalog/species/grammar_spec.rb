...
describe Importers::Bolton::Catalog::Species::Grammar do
  ...
  describe "parsing notes" do
    it "should parse a note" do
      expect(@grammar.parse('[Note. ]').value_with_matched_text_removed).to eq({:type => :note, :text => '[Note. ]'})
    end
  end

  describe "History items" do

    it "should retain the matching text for reparsing" do
      expect(@grammar.parse('Type-material redescribed: Crawley, 1922c: 19', root: :history_item).value).to eq({
        type_material_redescribed: {:references => [{:author_names => ['Crawley'], :year => '1922c', :pages => '19', matched_text: "Crawley, 1922c: 19"}]},
        matched_text: 'Type-material redescribed: Crawley, 1922c: 19'
      })
    end

    it "should parse redescription of type-material" do
      expect(@grammar.parse('Type-material redescribed: Crawley, 1922c: 19', :root => :history_item).value_with_matched_text_removed).to eq({
    ...
    it "should handle a history item with <recte>" do
      expect(@grammar.parse('Type-series referred to <i>testaceipes</i>-group (<i>recte</i> <i>terebrans</i>-group): McArthur & Adams, 1996: 41.', :root => :history_item).value_with_matched_text_removed).to eq({
        :text => [
    ...
    describe "Homonyms" do
      it "should parse a replacement name" do
        expect(@grammar.parse('Replacement name for <i>crinita</i> Wheeler, W.M., above. [Junior primary homonym of <i>Formica foetida</i> Linnaeus, 1758: 582.]', :root => :history_item).value_with_matched_text_removed).to eq({
          :replacement_for => {
      ...
      it "should parse a junior primary homonym with a replacement name" do
        expect(@grammar.parse('[Junior primary homonym of <i>alpina</i> Wheeler, above.] Replacement name: <i>mccooki</i> McCook, 1879: 187', :root => :history_item).value_with_matched_text_removed).to eq({
          :homonym_of => {
            :primary_or_secondary => :primary,
      ...
      describe "Unresolved homonyms" do
        it "should parse an unresolved homonym" do
          expect(@grammar.parse(%{Unresolved junior primary homonym of <i>apicalis</i> Latreille, 1802c: 204, above}, :root => :history_item).value_with_matched_text_removed).to eq(
            {:homonym_of => {
              :primary_or_secondary => :primary,
              :unresolved => true,
    ...
    it "should parse 'also described as new'" do
      expect(@grammar.parse('[Also described as new by Heer, 1850: 142.]', :root => :history_item).value_with_matched_text_removed).to eq({
    ...
    describe "First available use of" do
      it "should parse first available use of" do
        expect(@grammar.parse('First available use of <i>Forelius maccooki</i> r. <i>fiebrigi</i> var. <i>breviscapa</i> Forel, 1913l: 241; unavailable name',
                      :root => :history_item).value_with_matched_text_removed).to eq({
      ...
      it "should parse first available use of when that phrase does not appear" do
        expect(@grammar.parse('<i>Forelius maccooki</i> r. <i>fiebrigi</i> var. <i>breviscapa</i> Forel, 1913l: 241; unavailable name',
                      :root => :history_item).value_with_matched_text_removed[:first_available_use_of]).not_to be_nil
      end
      it "should parse first available use of with a form" do
        expect(@grammar.parse('<i>Forelius maccooki</i> r. <i>fiebrigi</i> var. <i>breviscapa</i> Forel, 1913l: 241 (w.); unavailable name',
                      :root => :history_item).value_with_matched_text_removed[:first_available_use_of]).not_to be_nil
      end
    end
    ...
    describe "Nomen nudum" do
      it "should parse a nomen nudum" do
        expect(@grammar.parse('<i>Nomen nudum</i>', :root => :history_item).value_with_matched_text_removed).to eq({:nomen_nudum => {}})
      end
      it "should parse a nomen nudum with attribution" do
        expect(@grammar.parse('<i>Nomen nudum</i>, attributed to Burmeister', :root => :history_item).value_with_matched_text_removed).to eq({:nomen_nudum => {:attributed_to => 'Burmeister'}})
      end
    end

    it "should parse references as history items" do
      expect(@grammar.parse('Mayr, 1886d: 432 (q.m.); Forel, 1886b: xxxix (w.)',
                    :root => :history_item).value_with_matched_text_removed).to eq({
        :references => [
    ...
    it "should parse first available replacement for a species epithet" do
      expect(@grammar.parse('First available replacement name for <i>picea</i> Nylander: Donisthorpe, 1918a: 9',
                    :root => :history_item).value_with_matched_text_removed).to eq({
    ...
    it "should parse first available replacement for a species name" do
      expect(@grammar.parse('First available replacement name for <i>Formica picea</i> Nylander: Donisthorpe, 1918a: 9',
                    :root => :history_item).value_with_matched_text_removed).to eq({
        :first_available_replacement_for => {
          :genus_name => 'Formica',
          :species_epithet => 'picea',
    ...
    it "should parse nomina nuda being referred here 'by'" do
      expect(@grammar.parse("Material of the <i>nomen nudum</i> <i>lucidula</i> referred here by Donisthorpe, 1915d: 83",
                     :root => :history_item).value_with_matched_text_removed).to eq({
    ...
    it "should parse nomen nudum material referred here, with colon" do
      expect(@grammar.parse(%{Material of the <i>nomen nudum</i> <i>cinereoglebaria</i> referred here: Dlussky & Pisarski, 1971: 157},
                     :root => :history_item).value_with_matched_text_removed).to eq({
        :nomen_nudum_material_referred_here => {
    ...
    describe "material referred here" do
      it "should parse material of unavailable names referred here" do
        expect(@grammar.parse(%{Material of the unavailable names <i>transversa</i>, <i>clara</i> referred here by Dlussky, 1967a: 77},
                      :root => :history_item).value_with_matched_text_removed).to eq({
      ...
      it "should parse material of one unavailable name referred here" do
        expect(@grammar.parse('Material of the unavailable name <i>pullula</i> referred here: Creighton, 1950a: 509',
                      :root => :history_item).value_with_matched_text_removed).to eq({
          :material_of_unavailable_names_referred_here => {
    ...
    describe "Raised to species" do
      it "should parse raised to species and material of unavailable name referred here" do
        expect(@grammar.parse(%{Raised to species and material of the unavailable name <i>transversa</i> referred here by Dlussky, 1967a: 77},
                      :root => :history_item).value_with_matched_text_removed).to eq({
          :raised_to_species => {
      ...
      it "should parse being raised to species" do
        expect(@grammar.parse('Raised to species: Bolton & Fisher, 2000: 257',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :raised_to_species => {
      ...
      it "should parse being raised to species and senior synonym of" do
        expect(@grammar.parse('Raised to species and senior synonym of <i>tucumanus</i>: Bolton & Fisher, 2000: 257',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :raised_to_species => {
            :references => [{:author_names => ['Bolton', 'Fisher'], :year => '2000', :pages => '257'}],
      ...
      it "should parse being revived to species and senior synonym of" do
        expect(@grammar.parse('Revived status as species and senior synonym of <i>vetula</i>: Creighton, 1950a: 457',
                       :root => :history_item).value_with_matched_text_removed).to eq({
      ...
      it "should parse being revived from synonymy and raised to species" do
        expect(@grammar.parse('Revived from synonymy and raised to species: Kusnezov, 1957b: 16 (in key)',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :raised_to_species => {
    ...
    describe "Revived from synonymy" do
      it "should parse being revived from synonymy, raised to species, and also a senior synonym" do
        expect(@grammar.parse('Revived from synonymy, raised to species and senior synonym of <i>lecontei</i>: Francoeur, 1973: 172',
                       :root => :history_item).value_with_matched_text_removed).to eq({
      ...
      it "should parse revived from synonymy and senior synonym" do
        expect(@grammar.parse('Revived from synonymy and senior synonym of <i>sublucida</i>: Buren, 1968a: 28',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :revived_from_synonymy => {
      ...
      it "should parse being revived from synonymy" do
        expect(@grammar.parse('Revived from synonymy: Kusnezov, 1957b: 16 (in key)',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :revived_from_synonymy => {
            :references => [{:author_names => ['Kusnezov'], :year => '1957b', :pages => '16 (in key)'}],
          }
      ...
      it "should parse being revived from synonymy with status as species" do
        expect(@grammar.parse('Revived from synonymy, with status as species: Dlussky, 1965a: 28',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :revived_from_synonymy => {
      ...
      it "should parse being revived from synonymy and status as species" do
        expect(@grammar.parse('Revived from synonymy and status as species: Kusnezov, 1957b: 16 (in key)',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :revived_from_synonymy => {
            :status_as_species => true,
            :references => [{:author_names => ['Kusnezov'], :year => '1957b', :pages => '16 (in key)'}],
      ...
      it "should parse being revived from synonymy as subspecies of" do
        expect(@grammar.parse('Revived from synonymy as subspecies of <i>fusca</i>: Kusnezov, 1957b: 16 (in key)',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :revived_from_synonymy => {
    ...
    describe "Revived status as species" do
      it "should parse revived status as species" do
        expect(@grammar.parse('Revived status as species: Bernard, 1967: 298',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :revived_status_as_species => {
      ...
      it "should parse revived status as species and senior synonym of" do
        expect(@grammar.parse('Revived status as species and senior synonym of <i>goesswaldi</i>, <i>naefi</i>, <i>tamarae</i>: Seifert, 2000a: 543',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :revived_status_as_species => {
    ...
    describe "unavailable names" do
      it "should parse an unavailable name" do
        expect(@grammar.parse('Unavailable name (Shattuck, 1994: 97); material referred to <i>grandis</i> by Cuezzo, 2000: 242',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :unavailable_name => {
      ...
      it "should parse an unavailable name with material referred to a taxon with authorship" do
        expect(@grammar.parse('Unavailable name (Shattuck, 1994: 97); material referred to <i>grandis</i> Forel (above) by Cuezzo, 2000: 242',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :unavailable_name => {
            :references => [{:author_names => ['Shattuck'], :year => '1994', :pages => '97'}],
            :material_referred_to => {
      ...
      it "should parse an unavailable name published as junior synonym" do
        expect(@grammar.parse('Unavailable name (published as junior synonym; Bolton, 1995b: 190)',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :unavailable_name => {
      ...
      it "should handle an unavailable name without parenthetical phrase" do
        expect(@grammar.parse('Unavailable name; material referred to <i>rufa</i> by Yarrow, 1955a: 4',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :unavailable_name => {
            :material_referred_to => {
      ...
      it "should handle an unavailable name without a reference" do
        expect(@grammar.parse('Unavailable name', :root => :history_item).value_with_matched_text_removed).to eq({
          :unavailable_name => {}
        })
      end
      it "should handle an unavailable name with just a reference" do
        expect(@grammar.parse('Unavailable name (Bolton, 1995b: 193)', :root => :history_item).value_with_matched_text_removed).to eq({
          :unavailable_name => {
    ...
    describe "junior synonym ofs" do
      it "should handle 'Provisional'" do
        expect(@grammar.parse('Provisional junior synonym of <i>picea</i> Nylander: Emery, 1925b: 249',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :synonym_ofs => [{
      ...
      it "should handle 'probable'" do
        expect(@grammar.parse('Probable synonym of <i>picea</i> Nylander: Emery, 1925b: 249',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :synonym_ofs => [{
            :species_epithet => 'picea', :authorship => [{:author_names => ['Nylander']}],
            :probable => true,
      ...
      it "should handle 'possible'" do
        expect(@grammar.parse('Possible junior synonym of <i>picea</i> Nylander: Emery, 1925b: 249',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :synonym_ofs => [{
            :species_epithet => 'picea', :authorship => [{:author_names => ['Nylander']}],
            :possible => true,
      ...
      it "should work with it doesn't say 'junior'" do
        expect(@grammar.parse('Synonym of <i>subnuda</i>: Creighton, 1950a: 469',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :synonym_ofs => [{
      ...
      it "should parse a junior synonym clause" do
        expect(@grammar.parse('Junior synonym of <i>nigriventris</i> Forel: Bolton & Fisher, 2000: 257',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :synonym_ofs => [{
            :junior_or_senior => :junior,
      ...
      it "should parse a multiple junior synonym clause" do
        expect(@grammar.parse('Junior synonym of <i>nigriventris</i>: Bolton & Fisher, 2000: 257; of <i>mccooki</i>: Wheeler, 2002: 6',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :synonym_ofs => [{
            :junior_or_senior => :junior,
      ...
      it "should parse a fossil junior synonym clause without bothering about the fossil status" do
        expect(@grammar.parse('Junior synonym of *<i>nigriventris</i>: Bolton & Fisher, 2000: 257',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :synonym_ofs => [{
            :junior_or_senior => :junior,
            :species_epithet => 'nigriventris',
      ...
      it "should handle a junior synonym that's an available replacement name" do
        expect(@grammar.parse('Junior synonym of <i>foetida</i> Buckley: Wheeler, W.M. 1901b: 520 and hence first available replacement name for <i>Formica foetida</i> Buckley, 1866: 167. [Junior primary homonym of <i>Formica foetida</i> Linnaeus, 1758: 582.]',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :synonym_ofs => [{
            :junior_or_senior => :junior,
    ...
    it "should parse 'relationship with'" do
      expect(@grammar.parse('Relationship with <i>densiventris</i>: Cole, 1954a: 89',
                     :root => :history_item).value_with_matched_text_removed).to eq({
        :relationship_with => {
    ...
    describe "senior synonym of" do
      it "should parse a very simple one" do
        expect(@grammar.parse('Senior synonym of <i>obscurata</i>: Cuezzo, 2000: 235',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :senior_synonym_ofs => [{
      ...
      it "should parse" do
        expect(@grammar.parse('Senior synonym of <i>obscurata</i>, <i>instabilis</i>: Cuezzo, 2000: 235',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :senior_synonym_ofs => [{
      ...
      it "should handle a junior synonym with an authorship" do
        expect(@grammar.parse('Senior synonym of <i>wheeleri</i> Stitz: Cuezzo, 2000: 235',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :senior_synonym_ofs => [{
      ...
      it "should handle 'senior synonym of x (and its junior synonyms y, z)" do
        expect(@grammar.parse('Senior synonym of <i>glauca</i> (and its junior synonyms <i>caucasica</i>, <i>katuniensis</i>, <i>montivaga</i>): Atanassov & Dlussky, 1992: 267',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :senior_synonym_ofs => [{
            :junior_synonyms => [{
      ...
      it "should handle 'senior synonym of x (and its junior synonym y)" do
        expect(@grammar.parse('Senior synonym of <i>glauca</i> (and its junior synonym <i>caucasica</i>): Atanassov & Dlussky, 1992: 267',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :senior_synonym_ofs => [{
            :junior_synonyms => [{
              :species_epithet => 'glauca',
...
"Senior synonym of <i>fiebrigi</i>, <i>pilipes</i> and material of the unavailable name <i>carmelitana</i> referred here: Cuezzo, 2000: 229",
      ...
      it "should handle senior synonym with material from multiple unavailable names referred here" do
        expect(@grammar.parse('Senior synonym of <i>bipilosa</i> and material of the unavailable names <i>clara</i>, <i>fallax</i> referred here: Dlussky, 1965a: 28',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :senior_synonym_ofs => [{
            :junior_synonyms => [
      ...
      it "should handle senior synonym with material from multiple unavailable names referred here with reference note" do
        expect(@grammar.parse('Senior synonym of <i>laevidens</i>, <i>nana</i>: Wilson, 1958a: 142; of <i>cephalotes</i> (and its junior synonym <i>maculata</i>): Brown, 1958h: 14; of <i>fortis</i>, <i>foveolata</i>, <i>minor</i>, <i>obscura</i>, and material of the unavailable names <i>howensis</i>, <i>norfolkensis</i>, <i>pallens</i>, <i>queenslandica</i> referred here: Brown, 1960a: 167 (these previously provisional synonyms in Brown, 1958h: 13)', :root => :history_item).value_with_matched_text_removed[:senior_synonym_ofs]).not_to be_nil
      end
...
"Senior synonym of <i>testaceus</i>: Cuezzo, 2000: 261; of <i>analis</i>: Ward, 2005: 9",
    ...
    describe "combinations in" do
      it "should parse a 'combination in' clause" do
        expect(@grammar.parse('Combination in <i>Forelius</i>: Shattuck, 1992c: 95',
                       :root => :history_item).value_with_matched_text_removed).to eq({:combinations_in => [
          {:genus_name => 'Forelius',
      ...
      it "should parse a 'combination in' clause with subgenus" do
        expect(@grammar.parse('Combination in <i>Formica (Proformica)</i>: Wheeler, W.M. 1913f: 539',
                       :root => :history_item).value_with_matched_text_removed).to eq({:combinations_in => [
      ...
      it "should parse a multiple 'combination in' clause" do
        expect(@grammar.parse('Combination in <i>Forelius</i>: Shattuck, 1992c: 95; in <i>Atta</i>: Wheeler, G.C. 1986g: 13',
                       :root => :history_item).value_with_matched_text_removed).to eq({:combinations_in => [
          {:genus_name => 'Forelius',
           :references => [{:author_names => ['Shattuck'], :year => '1992c', :pages => '95'}]},
          {:genus_name => 'Atta',
    ...
    describe "Subspecies ofs" do
      it "should parse a 'subspecies of' clause" do
        expect(@grammar.parse('Subspecies of <i>neorufibarbis</i> Forel: Creighton, 1950a: 537',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :subspecies_ofs => [{
      ...
      it "should parse multiple 'subspecies of' clauses" do
        expect(@grammar.parse('Subspecies of <i>neorufibarbis</i>: Creighton, 1950a: 537; of <i>rufa</i>: Walker, 1999: 100',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :subspecies_ofs => [
      ...
      it "should parse a fossil 'subspecies of' clause" do
        expect(@grammar.parse('Subspecies of *<i>neorufibarbis</i>: Creighton, 1950a: 537',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :subspecies_ofs => [{
      ...
      it "should parse a subspecies_of clause with 'material referred to'" do
        expect(@grammar.parse('Subspecies of <i>whymperi</i> and material of the unavailable name <i>hybrida</i> referred here: Creighton, 1950a: 510',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :subspecies_ofs => [{
    ...
    describe "Currently subspecies of" do
      it "parse currently subspecies of" do
        expect(@grammar.parse('Currently subspecies of <i>obscuriventris</i>: Creighton, 1950a: 494.',
                       :root => :history_item).value_with_matched_text_removed).to eq({
    ...
    describe "status as species" do
      it "should parse a 'status as species'" do
        expect(@grammar.parse('Status as species: Creighton, 1950a: 537',
                       :root => :history_item).value_with_matched_text_removed).to eq({:status_as_species => {
          :references => [{:author_names => ['Creighton'], :year => '1950a', :pages => '537'}]
        }})
      end
    end
    ...
    describe "Material referred to" do
      it "should parse 'material referred to' by itself" do
        expect(@grammar.parse('Material referred to <i>rufa</i> by Yarrow, 1955a: 4',
                      :root => :history_item).value_with_matched_text_removed).to eq({
          :material_referred_to => {
            :species_epithet => 'rufa',
            :references => [{:author_names => ['Yarrow'], :year => '1955a', :pages => '4'}]
      ...
      it "should handle a colon instead of 'by'" do
        expect(@grammar.parse('Material referred to <i>gnava</i>: Wheeler, W.M. 1913f: 518',
                      :root => :history_item).value_with_matched_text_removed).to eq({
          :material_referred_to => {
    ...
    describe "Misspellings" do
      it "should parse a misspelling" do
        expect(@grammar.parse('Misspelled as <i>paucistriatus</i> by Kempf, 1972a: 109',
                       :root => :history_item).value_with_matched_text_removed).to eq({
      ...
      it "should parse multiple misspellings" do
        expect(@grammar.parse('Misspelled as <i>fukali</i> by Emery, 1925b: 257 and as <i>fukalii</i> by Wu, 1990: 5',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :misspellings => [
      ...
      it "should parse a misspelling with slightly different phrasing" do
        expect(@grammar.parse('Name misspelled <i>subsericeorufibarbis</i> by Emery, 1925b: 250',
                       :root => :history_item).value_with_matched_text_removed).to eq({
    ...
    describe "green entries" do
      it "should parse an unidentifiable taxon" do
        expect(@grammar.parse('Unidentifiable taxon: Bolton, 1995b: 190',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :unidentifiable => {
            :references => [{:author_names => ['Bolton'], :year => '1995b', :pages => '190'}]
          }
      ...
      it "should parse an unidentifiable as to genus" do
        expect(@grammar.parse('Unidentifiable to genus; <i>incertae sedis</i> in <i>Formica</i>: Bolton, 1995b: 190',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :unidentifiable => {
            :references => [{:author_names => ['Bolton'], :year => '1995b', :pages => '190'}]
          }
      ...
      it "should parse an unidentifiable taxon incertae sedis in Formica" do
        expect(@grammar.parse('Unidentifiable taxon, <i>incertae sedis</i> in <i>Formica</i>: Seifert, 2004: 32',
                       :root => :history_item).value_with_matched_text_removed).to eq({
          :unidentifiable => {
    ...
    describe "see also" do
      it "should handle a normal one" do
        expect(@grammar.parse('See also: Cuezzo, 2000: 253', :root => :history_item).value_with_matched_text_removed).to eq({
          :see_also => {
      ...
      it "should handle 'see also comment in'" do
        expect(@grammar.parse('See also comment in Cuezzo, 2000: 253', :root => :history_item).value_with_matched_text_removed).to eq({
          :see_also => {
      ...
      it "should handle 'See:'" do
        expect(@grammar.parse('See: Cuezzo, 2000: 253', :root => :history_item).value_with_matched_text_removed).to eq({
          :see_also => {
            :text => 'See:',
            :references => [{:author_names => ['Cuezzo'], :year => '2000', :pages => '253'}]
          }
      ...
      it "should 'See also comment in... " do
        expect(@grammar.parse('See also comment in Seifert, 2002b: 267', :root => :history_item).value_with_matched_text_removed).to eq({
          :see_also => {
            :text => 'See also comment in',
    ...
    it "should parse 'Current subspecies'" do
      expect(@grammar.parse("Current subspecies: nominal plus <i>alpina</i>, <i>whymperi</i> (unresolved junior homonym)",
                     :root => :current_subspecies).value_with_matched_text_removed).to eq({
  ...
  describe "'see under' lines" do
    describe "species 'see under' lines" do
      it "should detect 'see under' lines" do
        expect(@grammar.parse("<i>eidmanni</i> Goetsch, 1933; see under <i>TAPINOMA</i>.").value_with_matched_text_removed).to eq({:type => :species_see_under})
      end
      it "should detect 'see under' lines" do
        expect(@grammar.parse("<i>emeryi</i> Forel, 1915: see under <i>PSEUDOLASIUS</i>.").value_with_matched_text_removed).to eq({:type => :species_see_under})
      end
      ...
      it "should handle a bracketed note after the authorship'" do
        ...
        expect(@grammar.parse(line).value[:type]).to eq(:species_see_under)
      end
      it "should handle a bracketed note after the authorship'" do
    ...
    describe "Genus see-under lines" do
      it "should detect genus 'see under' lines" do
        expect(@grammar.parse("<i>FLORENCEA</i>: see under <i>POLYRHACHIS</i>.").value_with_matched_text_removed).to eq({:type => :genus_see_under})
      end
  ...
  it "should parse an unparseable senior synonym list with a parenthesis, followed by a regular history item" do
    expect(@grammar.parse("Senior synonym of *<i>antiqua</i>, *<i>baltica</i>, *<i>parvula</i>: Dlussky, 2002a: 292; (replacement name for *<i>parvula</i> Dlussky, proposed subsequent to Dlussky's 2002a synonymy and hence automatic junior synonym). See also: Dlussky, 1967b: 81.", :root => :history).value_with_matched_text_removed).to eq({
      :history => [
        {:senior_synonym_ofs => [{
          :junior_synonyms => [
  ...
  describe "Genus header" do
    it "should parse a genus header" do
      expect(@grammar.parse("*<i>FALLOMYRMA</i>").value).to eq({:type => :genus_header, :name => 'Fallomyrma'})
    end
    it "should parse an ichnotaxon" do
      expect(@grammar.parse("*<i>ATTAICHNUS</i> (ichnotaxon)").value).to eq({:type => :genus_header, :name => 'Attaichnus'})
    end
    it "should parse a fossil with italicized phrase" do
      expect(@grammar.parse("*<i>MYRMICIUM</i> (Symphyta)").value).to eq({:type => :genus_header, :name => 'Myrmicium'})
    end
  end

  describe "subspecies lists" do
    it "should parse a subspecies list item that's an unresolved junior homonym" do
      expect(@grammar.parse("<i>alpina</i> (unresolved junior homonym)", :root => :subspecies_list_item).value).to eq(
  ...
  it "should parse the catalog header" do
    expect(@grammar.parse("CATALOGUE OF SPECIES-GROUP TAXA").value).to eq({:type => :catalog_header})
  end
  ...
  describe "Locality" do
    ...
    it "should parse locality" do
      expect(@grammar.parse('BRAZIL', :root => :locality).value).to eq('Brazil')
    end
    it "should handle a space and a ?" do
      expect(@grammar.parse('EGYPT ?', :root => :locality).value).to eq('Egypt ?')
    end
    it "should handle a ?" do
        expect(@grammar.parse('EGYPT?', :root => :locality).value).to eq('Egypt?')
    end
    it "should parse U.S.A." do
      expect(@grammar.parse('U.S.A.', :root => :locality).value).to eq('U.S.A.')
    end
    it "should parse U.S.A.?" do
      expect(@grammar.parse('U.S.A.?', :root => :locality).value).to eq('U.S.A.?')
    end
    it "should parse U.S.A. (Eocene)" do
      expect(@grammar.parse('U.S.A. (Eocene)', :root => :locality).value).to eq('U.S.A. (Eocene)')
    end
    ...
    it "should parse a two-word locality with a slash (and a question mark!)" do
      expect(@grammar.parse('CHINA/KASHMIR?', :root => :locality).value).to eq('China/Kashmir?')
    end
    it "should parse and lowercase accented characters" do
      expect(@grammar.parse('SÃO TOMÉ & PRÍNCIPE (São Tomé I.)', :root => :locality).value).to eq("São Tomé & Príncipe (São Tomé I.)")
    end
    it "should parse a two-word locality with parenthesis" do
      expect(@grammar.parse('BALTIC AMBER (Eocene)', :root => :locality).value).to eq('Baltic Amber (Eocene)')
    end
    it "should parse a locality with two-word parenthesis" do
      expect(@grammar.parse('UKRAINE (Rovno Amber)', :root => :locality).value).to eq('Ukraine (Rovno Amber)')
    end
    it "should parse a locality in quotes" do
      expect(@grammar.parse('"Grandes Indes"', :root => :locality).value).to eq('"Grandes Indes"')
    end
    it "should parse 'no locality given'" do
      expect(@grammar.parse('no locality given', :root => :locality).value).to eq('no locality given')
    end
    it "should parse 'no locality'" do
      expect(@grammar.parse('no locality', :root => :locality).value).to eq('no locality')
    end
    it "should parse 'error in locality'" do
      expect(@grammar.parse('"South America"; locality in error', :root => :locality).value).to eq('"South America"; locality in error')
    end
    it "should parse '(locality in error)'" do
      expect(@grammar.parse('INDONESIA (Sumatra) (locality in error)', :root => :locality).value).to eq('Indonesia (Sumatra) (locality in error)')
    end
  end
