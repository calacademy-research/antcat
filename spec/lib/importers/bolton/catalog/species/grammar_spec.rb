...
describe Importers::Bolton::Catalog::Species::Grammar do
  ...
  describe "parsing notes" do
    it "should parse a note" do
      @grammar.parse('[Note. ]').value_with_matched_text_removed.should == {:type => :note, :text => '[Note. ]'}
    end
  end
  ...
  describe "History items" do
    ...
    it "should retain the matching text for reparsing" do
      @grammar.parse('Type-material redescribed: Crawley, 1922c: 19', root: :history_item).value.should == {
    ...
    it "should parse redescription of type-material" do
      @grammar.parse('Type-material redescribed: Crawley, 1922c: 19', :root => :history_item).value_with_matched_text_removed.should == {
    ...
    it "should handle a history item with <recte>" do
      @grammar.parse('Type-series referred to <i>testaceipes</i>-group (<i>recte</i> <i>terebrans</i>-group): McArthur & Adams, 1996: 41.', :root => :history_item).value_with_matched_text_removed.should == {
        :text => [
    ...
    describe "Homonyms" do
      it "should parse a replacement name" do
        @grammar.parse('Replacement name for <i>crinita</i> Wheeler, W.M., above. [Junior primary homonym of <i>Formica foetida</i> Linnaeus, 1758: 582.]', :root => :history_item).value_with_matched_text_removed.should == {
          :replacement_for => {
      ...
      it "should parse a junior primary homonym with a replacement name" do
        @grammar.parse('[Junior primary homonym of <i>alpina</i> Wheeler, above.] Replacement name: <i>mccooki</i> McCook, 1879: 187', :root => :history_item).value_with_matched_text_removed.should == {
          :homonym_of => {
            :primary_or_secondary => :primary,
      ...
      describe "Unresolved homonyms" do
        it "should parse an unresolved homonym" do
          @grammar.parse(%{Unresolved junior primary homonym of <i>apicalis</i> Latreille, 1802c: 204, above}, :root => :history_item).value_with_matched_text_removed.should ==
            {:homonym_of => {
              :primary_or_secondary => :primary,
              :unresolved => true,
    ...
    it "should parse 'also described as new'" do
      @grammar.parse('[Also described as new by Heer, 1850: 142.]', :root => :history_item).value_with_matched_text_removed.should == {
    ...
    describe "First available use of" do
      it "should parse first available use of" do
        @grammar.parse('First available use of <i>Forelius maccooki</i> r. <i>fiebrigi</i> var. <i>breviscapa</i> Forel, 1913l: 241; unavailable name',
                      :root => :history_item).value_with_matched_text_removed.should == {
      ...
      it "should parse first available use of when that phrase does not appear" do
        @grammar.parse('<i>Forelius maccooki</i> r. <i>fiebrigi</i> var. <i>breviscapa</i> Forel, 1913l: 241; unavailable name',
                      :root => :history_item).value_with_matched_text_removed[:first_available_use_of].should_not be_nil
      end
    ...
    describe "Nomen nudum" do
      it "should parse a nomen nudum" do
        @grammar.parse('<i>Nomen nudum</i>', :root => :history_item).value_with_matched_text_removed.should == {:nomen_nudum => {}}
      end
      it "should parse a nomen nudum with attribution" do
        @grammar.parse('<i>Nomen nudum</i>, attributed to Burmeister', :root => :history_item).value_with_matched_text_removed.should == {:nomen_nudum => {:attributed_to => 'Burmeister'}}
      end
    end
    ...
    describe "unavailable names" do
      ...
      it "should handle an unavailable name without a reference" do
        @grammar.parse('Unavailable name', :root => :history_item).value_with_matched_text_removed.should == {
          :unavailable_name => {}
        }
      end
      it "should handle an unavailable name with just a reference" do
        @grammar.parse('Unavailable name (Bolton, 1995b: 193)', :root => :history_item).value_with_matched_text_removed.should == {
          :unavailable_name => {
...
"Senior synonym of <i>fiebrigi</i>, <i>pilipes</i> and material of the unavailable name <i>carmelitana</i> referred here: Cuezzo, 2000: 229",
      ...
      it "should handle senior synonym with material from multiple unavailable names referred here with reference note" do
        @grammar.parse('Senior synonym of <i>laevidens</i>, <i>nana</i>: Wilson, 1958a: 142; of <i>cephalotes</i> (and its junior synonym <i>maculata</i>): Brown, 1958h: 14; of <i>fortis</i>, <i>foveolata</i>, <i>minor</i>, <i>obscura</i>, and material of the unavailable names <i>howensis</i>, <i>norfolkensis</i>, <i>pallens</i>, <i>queenslandica</i> referred here: Brown, 1960a: 167 (these previously provisional synonyms in Brown, 1958h: 13)', :root => :history_item).value_with_matched_text_removed[:senior_synonym_ofs].should_not be_nil
      end
...
"Senior synonym of <i>testaceus</i>: Cuezzo, 2000: 261; of <i>analis</i>: Ward, 2005: 9",
    ...
    describe "combinations in" do
      it "should parse a 'combination in' clause" do
        @grammar.parse('Combination in <i>Forelius</i>: Shattuck, 1992c: 95',
                       :root => :history_item).value_with_matched_text_removed.should == {:combinations_in => [
          {:genus_name => 'Forelius',
    ...
    describe "status as species" do
      it "should parse a 'status as species'" do
        @grammar.parse('Status as species: Creighton, 1950a: 537',
                       :root => :history_item).value_with_matched_text_removed.should == {:status_as_species => {
          :references => [{:author_names => ['Creighton'], :year => '1950a', :pages => '537'}]
        }}
      end
    end
    ...
    describe "see also" do
      it "should handle a normal one" do
        @grammar.parse('See also: Cuezzo, 2000: 253', :root => :history_item).value_with_matched_text_removed.should == {
          :see_also => {
      ...
      it "should handle 'see also comment in'" do
        @grammar.parse('See also comment in Cuezzo, 2000: 253', :root => :history_item).value_with_matched_text_removed.should == {
          :see_also => {
      ...
      it "should handle 'See:'" do
        @grammar.parse('See: Cuezzo, 2000: 253', :root => :history_item).value_with_matched_text_removed.should == {
          :see_also => {
            :text => 'See:',
            :references => [{:author_names => ['Cuezzo'], :year => '2000', :pages => '253'}]
          }
      ...
      it "should 'See also comment in... " do
        @grammar.parse('See also comment in Seifert, 2002b: 267', :root => :history_item).value_with_matched_text_removed.should == {
          :see_also => {
            :text => 'See also comment in',
    ...
    it "should parse 'Current subspecies'" do
      @grammar.parse("Current subspecies: nominal plus <i>alpina</i>, <i>whymperi</i> (unresolved junior homonym)",
                     :root => :current_subspecies).value_with_matched_text_removed.should == {
  ...
  describe "'see under' lines" do
    describe "species 'see under' lines" do
      it "should detect 'see under' lines" do
        @grammar.parse("<i>eidmanni</i> Goetsch, 1933; see under <i>TAPINOMA</i>.").value_with_matched_text_removed.should == {:type => :species_see_under}
      end
      it "should detect 'see under' lines" do
        @grammar.parse("<i>emeryi</i> Forel, 1915: see under <i>PSEUDOLASIUS</i>.").value_with_matched_text_removed.should == {:type => :species_see_under}
      end
    ...
    describe "Genus see-under lines" do
      it "should detect genus 'see under' lines" do
        @grammar.parse("<i>FLORENCEA</i>: see under <i>POLYRHACHIS</i>.").value_with_matched_text_removed.should == {:type => :genus_see_under}
      end
  ...
  it "should parse an unparseable senior synonym list with a parenthesis, followed by a regular history item" do
    @grammar.parse("Senior synonym of *<i>antiqua</i>, *<i>baltica</i>, *<i>parvula</i>: Dlussky, 2002a: 292; (replacement name for *<i>parvula</i> Dlussky, proposed subsequent to Dlussky's 2002a synonymy and hence automatic junior synonym). See also: Dlussky, 1967b: 81.", :root => :history).value_with_matched_text_removed.should == {
      :history => [
        {:senior_synonym_ofs => [{
          :junior_synonyms => [
