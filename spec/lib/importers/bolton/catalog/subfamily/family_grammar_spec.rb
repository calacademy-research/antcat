...
describe Importers::Bolton::Catalog::Subfamily::FamilyGrammar do
  ...
  describe "Family-group headline" do
    it "should recognize a family group headline" do
      @grammar.parse(%{Formicariae Latreille, 1809: 124. Type-genus: <i>Formica</i>.}).value_with_matched_text_removed.should == {
  ...
  describe "Family-group protonym" do
    it "should recognize a family protonym" do
      @grammar.parse(%{*Formicariae Latreille, 1809: 124}, :root => :family_group_protonym).value_with_matched_text_removed.should == {
  ...
  it "should recognize the extinct genera incertae sedis list" do
    @grammar.parse(%{Genera (extinct) <i>incertae sedis</i> in Formicidae: *<i>Condylodon</i>, <i>Hypochira</i>.}).value_with_matched_text_removed.should ==
  ...
  describe "Unavailable family-group names" do
    ...
    it "should parse the details" do
      @grammar.parse(%{Alloformicinae Emery, 1925b: 9 [as "section" of Formicinae]. Section designated to include tribes Melophorini, Myrmelachistini and Plagiolepidini. Unavailable name; not based on genus rank taxon. Contained material referable to Formicinae: Bolton, 1994: 51.}, :root => :unavailable_family_group_name_detail).value_with_matched_text_removed.should == {
    ...
    it "should handle a colon after the taxon name (in other words, it's not a protonym)" do
      @grammar.parse(%{Promyrmicinae: Forel, 1917: 240 [incorrect expansion of the above unavailable name to include tribes Metaponini and Pseudomyrmini]. Unavailable name.}, :root => :unavailable_family_group_name_detail).value_with_matched_text_removed.should == {
        :type => :unavailable_family_group_name_detail,
