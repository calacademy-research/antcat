...
describe Protonyms::Operations::MoveItems do
  describe "#call" do
    describe 'moving history items' do
      ...
      let!(:history_item_1) { create :history_item, :taxt, protonym: from_protonym }
      let!(:history_item_2) { create :history_item, :taxt, protonym: from_protonym }
      let!(:history_item_3) { create :history_item, :taxt, protonym: from_protonym }
      ...
      context 'when moving some but not all items belonging to a protonym' do
        let!(:history_item_4) { create :history_item, :taxt, protonym: from_protonym }
      ...
      context 'when `to_protonym` has history items of its own' do
        ...
        before do
          create :history_item, :taxt, protonym: to_protonym
        end
