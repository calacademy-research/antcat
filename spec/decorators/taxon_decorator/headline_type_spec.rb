...
describe TaxonDecorator::HeadlineType do
  let(:species_name) { build_stubbed :species_name, name: 'Atta major' }

  describe "#call" do
    it "shows the type taxon" do
      taxon = build_stubbed :genus, type_name: species_name
    ...
    it "shows the type taxon with extra Taxt" do
      taxon = build_stubbed :genus, type_name: species_name, type_taxt: ', by monotypy'
      expect(described_class[taxon]).
  ...
  describe "#format_type_name" do
    ...
    let!(:taxon) { build_stubbed :genus, type_name: species_name }
