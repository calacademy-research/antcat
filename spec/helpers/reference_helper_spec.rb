...
describe ReferenceHelper do
  describe "formattng reference" do
    it "should format the reference" do
      ...
      reference = Factory(:reference, :authors => [author], :citation_year => "1874", :title => "Les fourmis de la Suisse",
    ...
    it "should add a period after the title if none exists" do
      reference = ward_reference_factory(:authors => "authors", :year => "year", :title => "No period", :citation => "citation")
    ...
    it "should not add a period after the title if there's already one" do
      reference = ward_reference_factory(:authors => "authors", :year => "year", :title => "With period.", :citation => "citation")
    ...
    it "should add a period after the citation if none exists" do
      reference = ward_reference_factory(:authors => "authors", :year => "year", :title => "title", :citation => "No period")
    ...
    it "should not add a period after the citation if there's already one" do
      reference = ward_reference_factory(:authors => "authors", :year => "year", :title => "title", :citation => "With period.")
  ...
  describe "formatting the date" do
    ...
    def make date
      @reference = ward_reference_factory(:date => date, :authors => "authors", :year => "year", :title => "title", :citation => "With period.")
    end
