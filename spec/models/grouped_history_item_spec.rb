...
describe GroupedHistoryItem, :relational_hi do
  ...
  describe '#taxt' do
    ...
    context 'with `COMBINATION_IN` items' do
      ...
      specify do
        expect(grouped_history_item.taxt).
          to eq "Combination in #{Taxt.tax(item.object_taxon.id)}: #{item.citation_taxt}; #{item.citation_taxt}."
      end
    end
    ...
    context 'with `JUNIOR_SYNONYM_OF` items' do
      ...
      specify do
        expect(grouped_history_item.taxt).
          to eq "Junior synonym of #{Taxt.prott(item.object_protonym.id)}: #{item.citation_taxt}; #{item.citation_taxt}."
      end
    end
    ...
    context 'with `SENIOR_SYNONYM` items' do
      ...
      specify do
        expect(grouped_history_item.taxt).
          to eq "Senior synonym of #{Taxt.prott(item.object_protonym.id)}: #{item.citation_taxt}; #{item.citation_taxt}."
      end
    end
    ...
    context 'with `SUBSPECIES_OF` items' do
      ...
      specify do
        expect(grouped_history_item.taxt).
          to eq "Subspecies of #{Taxt.prott(item.object_protonym.id)}: #{item.citation_taxt}; #{item.citation_taxt}."
      end
    end
  end
end
